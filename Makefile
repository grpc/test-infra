...
# other than DockerHub
IMAGE_PREFIX ?= ""
# Default pool of nodes where drivers should be scheduled
DRIVER_POOL ?= "drivers"
# Default pool of nodes where workers should be scheduled
WORKER_POOL ?= "workers-8core"
...
# Generate a defaults.yaml configuration file
configure:
	go run config/cmd/configure.go -version=${TEST_INFRA_VERSION} \
		-init-image-prefix=${INIT_IMAGE_PREFIX} \
		-image-prefix=${IMAGE_PREFIX} \
		-driver-version=${DRIVER_VERSION} \
		-worker-pool=${WORKER_POOL} \
		-driver-pool=${DRIVER_POOL} \
		config/defaults_template.yaml config/defaults.yaml
